...
The hypotheses `A > 0` and `B < 0` concern spectral moments. For the explicit profile `φ = 1_[1,2] − (1/2) 1_[3,4]`, the squared norm is `5/4`; its normalized spectral representative is `(2·1_[1,2] − 1_[3,4])/√5`. The definition `exampleProfile` is `2φ`. Scaling by a nonzero constant leaves Born probabilities unchanged.
...
This controls the density, its time derivative, and the current. The function `1/(1+x²)` is integrable on the real line. These estimates also make the current tend to zero at both spatial infinities.
...
The Fourier integral omits a normalization prefactor. In each representation, Born probabilities are integrals of squared modulus divided by the total squared norm. The position denominator is finite, positive, and time independent, and the normalized current is `j / totalMass`.
...
gives `A=u(t−s)/(2t)>0` and `B=u(s−t)/4<0`. Hence `arbitraryBands_backflow` works for every `0<a<b<d<e`, and `chosenCoefficient_backflow` proves backflow with this coefficient.
